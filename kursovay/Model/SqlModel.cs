...
namespace kursovay.Model
{
    public class SqlModel
    {
        ...
        public int Insert<T>(T value)
        {
            ...
            if (value is Postavki || value is Obuchenie)
            {
        ...
        public List<Personal> PersonalSelect(int skip, int count)
        {
            var personals = new List<Personal>();
            var mySqlDB = MySqlDB.GetDB();
            string query = $"SELECT * FROM `personal` LIMIT {skip},{count}";
            if (mySqlDB.OpenConnection())
            {
                using (MySqlCommand mc = new MySqlCommand(query, mySqlDB.sqlConnection))
                using (MySqlDataReader dr = mc.ExecuteReader())
        ...
        public List<ObucheniePer> SelectPersonalByVrach(Vrach selectVrachView)

        {
            var result = new List<ObucheniePer>();
            var mySqlDB = MySqlDB.GetDB();
            string query = $"SELECT * FROM obuchenie o LEFT JOIN personal p ON p.id = o.idPersonal WHERE o.idVrach = " + selectVrachView.ID;
            if (mySqlDB.OpenConnection())
            {
                using (MySqlCommand mc = new MySqlCommand(query, mySqlDB.sqlConnection))
                using (MySqlDataReader dr = mc.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        result.Add(new ObucheniePer
                        {
                            Personal = new Personal { ID = dr.GetInt32("idPersonal"), LastName = dr.GetString("LastName") , Name = dr.GetString("Name") , gender = dr.GetString("gender") },
        ...
        public List<Personal> SelectPersonalsByVrachs(Vrach vrach)
        {
            var result = new List<Personal>();
            var mySqlDB = MySqlDB.GetDB();
            string query = $"SELECT * FROM `obucheie` LEFT JOIN personal r ON idPersonal = r.id AND idVrach = " + vrach.ID;
            if (mySqlDB.OpenConnection())
            {
                using (MySqlCommand mc = new MySqlCommand(query, mySqlDB.sqlConnection))
                using (MySqlDataReader dr = mc.ExecuteReader())
